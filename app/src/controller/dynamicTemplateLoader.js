...
function invokeRenderTemplate(requestOptons, name, options) {
    ...
    if(templates.items[name] && typeof templates.items[name]=="function") {
...
module.exports = {
    ...
    initTemplates : function(dirPath, devMode, filesExt, compileCallback) {
        ...
        console.log("init templates is called ********************");
        console.log("file ext"+filesExt);
        var allFiles = [];
        ...
        allFiles.forEach(function (filePath) {
            ...
            var funcName = filePath.substring(dirPath.length+1,filePath.length);
            funcName = funcName.replace(/\\/g,"/");

            var supported = false;
            
            supported = funcName.length >= filesExt.length && funcName.substr(funcName.length - filesExt.length) == filesExt; //funcName.indexOf(filesExt, funcName.length - filesExt.length) !== -1 ? true : false;
            console.log("supported"+supported + " file"+funcName);
            if(supported){
