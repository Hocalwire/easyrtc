...
PhotoSwipeWrapper.lazyLoad10ImagesFromIndex = function(index,selector) {
    ...
    for(var i=sIndex;i<eIndex;i++){
        ClientUtils.lazyLoadImage(selector+" .carouselIndicator-"+i+" img");
    }
};
...
PhotoSwipeWrapper.trackingWithVideoPlay = function(){
    Proptiger.Services.TrackingService.sendGAEvent(
        'mgallery',
        'viewedFull',
...
PhotoSwipeWrapper.openInJWPlayer = function(source){
    ClientUtils.loadJWPlayer(function(){
        ClientUtils.playVideo(source, photoSwipeObject.currItem.src);
    });
};
