$.fn.extend({
	boxify: function(args) {
		var $this = $(this),
			...
			positions = [],
			singleItem=false;
		if(!(cont_width>(arg_width*2))){
			singleItem=true;
			arg_width=cont_width;
			cont_boxes=$boxes.length;
		}
		$this.css({
			margin: arg_margin / 2 + 'px ' + (singleItem ? '5' : cont_space / 2) + 'px',
			position: 'relative'
		});
		...
		$boxes.each(function(i, box) {
			...
			$box.css({
				...
				margin: singleItem ? '5px' : arg_margin / 2,
				padding: arg_padding,
				position: 'absolute',
					top: i >= cont_boxes ? positions[i-cont_boxes] : (singleItem ? positions[i-1] : 0),
					left: singleItem ? arg_margin : (arg_width + arg_margin) * Math.round(i % cont_boxes),
				width: singleItem ? '100%' : arg_width
			});
...
var initPhotoSwipeFromDOM = function(gallerySelector) {
    ...
    var parseThumbnailElements = function(el) {
        ...
        for(var i = 0; i < numNodes; i++) {
            ...
            if($(figureEl).attr("has-figure-child")){
            	figureEl = $(figureEl).find("figure")[0]
            }
            ...
            // create slide object
            item = {
                src: figureEl.getAttribute("data-url") ? figureEl.getAttribute("data-url") : linkEl.getAttribute('href'),
                shareUrl : linkEl.getAttribute('share-url'),
    ...
    var onThumbnailsClick = function(e) {
        ...
        if(!clickedListItem.hasAttribute("data-image-item")){
        	return;
        }
        if(!clickedListItem) {
            return;
        ...
        // alternatively, you may define index via data- attribute
        	var parentLevel = $(clickedListItem).attr("data-parent-level") ? parseInt($(clickedListItem).attr("data-parent-level")) : 1

        	var clickedGallery = parentLevel == 1 ? $(clickedListItem).parent()[0] : $(clickedListItem).parent().parent()[0];
        ...
        for (var i = 0; i < numChildNodes; i++) {
            ...
            if(childNodes[i] === (parentLevel==2 ? $(clickedListItem).parent()[0] : clickedListItem)) {
                index = nodeIndex;
                break;
            }
            nodeIndex++;
    ...
    $('.view_video').each(function(){  //init all videos
...
(function ($) {
    ...
    function showModal(options) {
        ...
        if (options.getVideoType() == options.VideoEnum.MP4)
            dialogContent = "<div id='dialog-content' class='modal-body'><video width='100%' src='" + options.link + "' controls autoplay></video></div>";

        else if (options.getVideoType() == options.VideoEnum.YOUTUBE)
            dialogContent = "<div id='dialog-content' class='modal-body modal-body-video'><div id='"+id+"'></div></div>";

        if(dialogContent)
            dialogContainer.append(dialogContent);

       
        if(options.getVideoType() == options.VideoEnum.YOUTUBE){
        ...
        if(window.pauseBulletin){
            window.pauseBulletin();
        }
    }
    ...
    $.fn.modalvideo = function (options) {
        ...
        $(this).each(function (i) {
            ...
            var link = $(this).attr("data-video-path");
