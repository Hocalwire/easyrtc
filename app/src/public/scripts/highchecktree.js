...
(function($){
    ...
    jQuery.fn.highCheckTree = function(settings){
        ...
        function getChildrenHtml(treesdata,parent){
            ...
            for(i = 0; i < len; i++){
                ...
                if(clen === 0){
                    ...
                    if(node.item.checked || (parent && parent.item.checked)){
                        checkClass = "checked";
                    } else {
               ...
               result += '<li class="item", rel="' + node.item.id + '" data-type="'+node.item.type+'" data-parent="'+node.item.parent+'" data-param-name="'+node.item.param_name+'"><div class="arrow ' + arrowClass + '"></div><div class="checkbox ' + checkClass + '"></div><label title="'+node.item.label+'">' + 
        ...
        function updateChildrenNodes($li, data, isExpanded) {
            if(data.children && data.children.length>0){
              var innerHtml = isExpanded ? '<ul>' : '<ul style="display:none;">';
                innerHtml += getChildrenHtml(data.children,data) + '</ul>';
                $li.append(innerHtml);  
            }
        ...
        container.off('mouseenter', 'label').on('mouseenter', 'label', function(){
         ...
         function expandMe(){
            var es = container.find(".checkbox");
            var tBucket = [];
            for(var i=0;i<es.length;i++){
                if($(es[i]).hasClass("half_checked") || $(es[i]).hasClass("checked")){
                   tBucket.push($(es[i]).parent().find(".arrow"));//.trigger("click");
                }
            }    
            for(var i=0;i<tBucket.length;i++){
                tBucket[i].trigger("click");
            }
         }
         expandMe();
